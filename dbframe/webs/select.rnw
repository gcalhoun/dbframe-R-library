...
<<Handle arguments and set up the local environment>>=
    if (!as.data.frame) warning("'select' will return a data.frame when called on a data.frame.  I'm ignoring your argument.")
    tablename <- "dataframe"
    require(RSQLite)
...
select(chickwts, c(averageweight = "avg(weight)"), group.by = c(thefeed = "feed"))
}
